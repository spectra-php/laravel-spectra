...
it('applies request-style filters in trackables endpoint', function () {
    $included = SpectraRequest::create([
        'provider' => 'openai',
        'model' => 'gpt-4o',
        'model_type' => 'text',
        'status_code' => 200,
    ...
    SpectraRequest::create([
        ...
        'finish_reason' => 'stop',
        'has_tool_calls' => false,
        'trace_id' => 'trace-other-provider',
        'prompt_tokens' => 10,
        'completion_tokens' => 5,
        'total_cost_in_cents' => 50,
        'trackable_type' => 'App\\Models\\Team',
        'trackable_id' => 2,
        'created_at' => now(),
    ]);
    ...
    expect($json['data'][0]['trackable_type'])->toBe('App\\Models\\User');
    expect($json['data'][0]['trackable_id'])->toBe(1);
    expect($json['data'][0]['requests'])->toBe(1);
    expect($json)->toHaveKey('available_tags');
    expect($json)->toHaveKey('available_finish_reasons');
    expect($json['available_tags'])->toContain('priority');
});
...
it('filters trackables by custom date range', function () {
    ...
    try {
        ...
        expect($json['total'])->toBe(1);
        expect($json['data'])->toHaveCount(1);
        expect($json['data'][0]['trackable_id'])->toBe(7);
        expect($json['summary']['total_requests'])->toBe(1);
    } finally {
        Carbon::setTestNow();
    }
});
