...
it('can calculate costs for a request', function () {
...
it('returns zero cost for unknown models', function () {
    ...
    expect($costs['completion_cost'])->toEqual(0.0);
    expect($costs['total_cost_in_cents'])->toEqual(0.0);
});
...
it('calculates non-zero cost for small token counts', function () {
    ...
    // 50 tokens at 60 cents/1M = 50 * 60 / 1,000,000 = 0.003 cents
    expect($costs['completion_cost'])->toEqual(0.003);

    // Verify total cost is approximately correct (accounts for floating point precision)
    expect($costs['total_cost_in_cents'])->toBeGreaterThan(0.004);
    expect($costs['total_cost_in_cents'])->toBeLessThan(0.005);

    // Verify cost is non-zero (the bug we're fixing)
    expect($costs['total_cost_in_cents'])->toBeGreaterThan(0);
});

it('can calculate costs with openai pricing tiers', function () {
    $calculator = new CostCalculator;
    ...
    expect($standardCosts['completion_cost'])->toEqual(1000.0);
    ...
    expect($batchCosts['completion_cost'])->toEqual(500.0);
...
it('can calculate costs with pricing tier as string', function () {
    $calculator = new CostCalculator;
    ...
    expect($costs['completion_cost'])->toEqual(500.0);
});
...
it('can get cost per token with pricing tier', function () {
    $calculator = new CostCalculator;
    ...
    // Standard: 250 cents per 1M = 0.00025 cents per token
    // Batch: 125 cents per 1M = 0.000125 cents per token
});
