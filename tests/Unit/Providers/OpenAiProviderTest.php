...
it('extracts usage from completions api response', function () {
    ...
    $metrics = $this->openAiProvider()->extractMetrics($response);
...
it('extracts content from completions api response', function () {
    ...
    $content = $this->openAiProvider()->extractResponse($response);
...
it('extracts model from completions api response', function () {
    ...
    $model = $this->openAiProvider()->extractModel($response);
...
it('extracts model from batch response', function () {
    ...
    $data = TextHandler::unwrapBatch($response);
    $model = (new TextHandler)->extractModelFromResponse($data);
...
it('extracts finish reason from completions api', function () {
    ...
    $finishReason = $this->openAiProvider()->extractFinishReason($response);
...
it('extracts image metrics via provider for image endpoint', function () {
    ...
    $metrics = $this->openAiProvider()->extractMetrics(
        $response,
...
it('returns empty metrics for unknown endpoint', function () {
    $metrics = $this->openAiProvider()->extractMetrics([], '/v1/unknown');
...
it('extracts model from embedding response', function () {
    ...
    expect($handler->extractModelFromResponse($response))->toBe('text-embedding-3-small');
});
...
it('resolves text model type for chat completions endpoint', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/chat/completions'))->toBe(ModelType::Text);
});

it('resolves text model type for responses endpoint', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/responses'))->toBe(ModelType::Text);
});

it('resolves image model type for image generations endpoint', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/images/generations'))->toBe(ModelType::Image);
});

it('resolves image model type for image edits endpoint', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/images/edits'))->toBe(ModelType::Image);
});

it('resolves embedding model type for embeddings endpoint', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/embeddings'))->toBe(ModelType::Embedding);
});

it('returns null model type for unknown endpoint', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/unknown'))->toBeNull();
});
...
it('resolves transcription handler for transcriptions endpoint', function () {
    $handler = $this->openAiProvider()->resolveHandler('/v1/audio/transcriptions');
...
it('resolves transcription handler for translations endpoint', function () {
    $handler = $this->openAiProvider()->resolveHandler('/v1/audio/translations');
...
it('extracts model from verbose transcription response', function () {
    ...
    expect($handler->extractModelFromResponse($response))->toBe('whisper-1');
});
...
it('resolves speech handler for speech endpoint', function () {
    $handler = $this->openAiProvider()->resolveHandler('/v1/audio/speech');
...
it('resolves stt model type for transcriptions endpoint', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/audio/transcriptions'))->toBe(ModelType::Stt);
});

it('resolves tts model type for speech endpoint', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/audio/speech'))->toBe(ModelType::Tts);
});
...
it('resolves video handler for /v1/videos endpoint', function () {
    $handler = $this->openAiProvider()->resolveHandler('/v1/videos');

    expect($handler)->toBeInstanceOf(VideoHandler::class);
});

it('resolves video handler for /v1/videos/{id} endpoint', function () {
    $handler = $this->openAiProvider()->resolveHandler('/v1/videos/video_abc123');

    expect($handler)->toBeInstanceOf(VideoHandler::class);
});

it('video handler returns video model type via provider', function () {
    expect($this->openAiProvider()->resolveModelType('/v1/videos'))->toBe(ModelType::Video);
});
...
it('resolves responses image handler for responses endpoint with image generation response', function () {
    ...
    $handler = $this->openAiProvider()->resolveHandler('/v1/responses', $response);
...
it('responses image handler extracts model', function () {
    ...
    expect($handler->extractModelFromResponse($response))->toBe('gpt-4o-2024-08-06');
});
...
it('extracts image metrics via provider for responses endpoint with image generation', function () {
    ...
    $metrics = $this->openAiProvider()->extractMetrics($response, '/v1/responses', []);
