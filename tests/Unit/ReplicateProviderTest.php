...
function replicateProvider(): Replicate
{
    return new Replicate;
}

it('returns replicate as provider', function () {
    expect(replicateProvider()->getProvider())->toBe('replicate');
});
...
it('returns correct hosts', function () {
    expect(replicateProvider()->getHosts())->toBe(['api.replicate.com']);
});

it('resolves image handler for predictions endpoint', function () {
    $handler = replicateProvider()->resolveHandler('/v1/models/stability-ai/sdxl/predictions');

    expect($handler)->toBeInstanceOf(ImageHandler::class);
});

it('returns null for non-predictions endpoint', function () {
    $handler = replicateProvider()->resolveHandler('/v1/something-else');
...
it('extracts model from prediction response', function () {
    ...
    expect($handler->extractModel($response))->toBe('stability-ai/stable-diffusion');
});
...
it('extracts output urls from prediction response', function () {
    ...
    expect($content)->toContain('output2.png');
});
