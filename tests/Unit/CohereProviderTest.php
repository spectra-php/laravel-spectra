...
function cohereProvider(): Cohere
{
    return new Cohere;
}

it('returns cohere as provider', function () {
    expect(cohereProvider()->getProvider())->toBe('cohere');
});
...
it('returns correct hosts', function () {
    expect(cohereProvider()->getHosts())->toBe(['api.cohere.com']);
});

it('resolves chat handler for v2 chat endpoint', function () {
    $handler = cohereProvider()->resolveHandler('/v2/chat');
...
it('extracts model from cohere response', function () {
    ...
    expect($handler->extractModel($response))->toBe('command-r-plus');
});
...
it('extracts metrics via provider', function () {
    ...
    $metrics = cohereProvider()->extractMetrics($response, '/v2/chat');
