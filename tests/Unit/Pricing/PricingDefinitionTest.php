...
it('should build a model definition with all fields', function () {
...
it('should build a model with multiple capabilities', function () {
    ...
    expect($array['can_generate_text'])->toBeTrue()
        ->and($array['can_generate_images'])->toBeTrue()
        ->and($array['can_generate_audio'])->toBeTrue()
        ->and($array['can_generate_video'])->toBeFalse();
});

it('should build a model with pricing tiers', function () {
    $model = new ModelDefinition('gpt-4o');
...
it('should build a model with cache write pricing', function () {
    ...
    expect($tiers[0]['cache_write_5m_price'])->toBe(375.0)
        ->and($tiers[0]['cache_write_1h_price'])->toBe(600.0);
});

it('should build a unit-based model with price per unit', function () {
    ...
    expect($array['type'])->toBe('image')
        ->and($array['pricing_unit'])->toBe('image')
        ->and($array['can_generate_images'])->toBeTrue()
        ->and($tiers[0]['price_per_unit'])->toBe(4000.0)
        ->and($tiers[0]['input_price'])->toBe(0.0)
        ->and($tiers[0]['output_price'])->toBe(0.0);
});

it('should build a model with cost shorthand for standard tier', function () {
...
it('should combine cost shorthand with additional tiers', function () {
    $model = new ModelDefinition('gpt-4o');
    ...
    expect($tiers)->toHaveCount(2)
        ->and($tiers[0]['tier'])->toBe('standard')
        ->and($tiers[1]['tier'])->toBe('batch');
});

it('should use cost shorthand with price per unit', function () {
    $model = new ModelDefinition('dall-e-3');
    $model->type('image')
        ->pricingUnit('image')
        ->canGenerateImages()
...
it('uses default values for type and pricing unit', function () {
    ...
    expect($array['type'])->toBe('text')
        ->and($array['pricing_unit'])->toBe('tokens');
});
...
it('provider pricing returns models from define method', function () {
    ...
    expect($models)->toHaveCount(1)
        ...
        ->and($models[0]->getInternalName())->toBe('test-model');
});
...
it('provider pricing toArray returns structured data', function () {
    ...
    expect($array)->toHaveKey('provider', 'my-provider')
        ->and($array)->toHaveKey('models')
        ->and($array['models'])->toHaveCount(1)
        ->and($array['models'][0])->toHaveKey('model')
        ->and($array['models'][0])->toHaveKey('tiers');
});
