...
function mistralProvider(): Mistral
{
    return new Mistral;
}

it('returns mistral as provider', function () {
    expect(mistralProvider()->getProvider())->toBe('mistral');
});
...
it('returns correct hosts', function () {
    expect(mistralProvider()->getHosts())->toBe(['api.mistral.ai', 'codestral.mistral.ai']);
});

it('resolves chat handler for mistral endpoint', function () {
    $handler = mistralProvider()->resolveHandler('/v1/chat/completions');
...
it('extracts metrics via provider', function () {
    ...
    $metrics = mistralProvider()->extractMetrics($response, '/v1/chat/completions');
